...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["name"]) && isset($_POST["comment"])) {
    ...
    $sql = "INSERT INTO comments1 (name, comment, parent_comment_id, audio_data) VALUES (?, ?, ?, ?)";
...
$sql = "SELECT c1.id, c1.name, c1.comment, c1.audio_data, c2.name AS parent_name, c2.comment AS parent_comment, c1.parent_comment_id FROM comments1 c1 LEFT JOIN comments1 c2 ON c1.parent_comment_id = c2.id WHERE c1.parent_comment_id IS NOT NULL";
...
<body>
    <div class="container">
        ...
        <p>Winter is coming, and the Recreation and Parks Department is working hard to mitigate the challenges that come with the colder months. Two key focus areas are tree maintenance, and mitigating impacts on cemeteries.<a> read more....</a></p>
        <p>The Department works proactively to mitigate weather impacts by conducting regular tree inspections and promptly addressing maintenance requests.

There are also dedicated emergency tree teams available after hours with chainsaws, trimmers, cherry pickers and other specialised equipment, ready to respond to unforeseen circumstances.

Since 1 July 2023, the Department has received 14 925 tree-related service requests and complaints – 355 during the adverse weather experienced in April 2024.</p>
    </div>
